...
const uint8_t  button_to_pin_new[] = 
{31,35,38,42,46,30,32,36,40,44,51,26,29,33,39,45,48,50,22,25,28,34,41,49,52,A13,17,18,24,27,37,47,A15,A12,A11,14,16,19,23,43,A14,A8,A7,5,4,15,6,A4,A6,A5,11,10,9,7,A1,A3,12,13,8,A0,A2}; 
...
void acquireButtons(){    // 1.68 us execution time  
...
void initButtons(){  
  ...
  for(uint8_t i=0; i<number_of_buttons; i++){      //Initialization loop
    ...
    if((button_to_pin[i] == 18) || (button_to_pin[i] == 19)){   // dont tread on 2nd serial port for now (until we reassign buttons) use to skip i = 27 and 35
      break;
    }
  
    pinMode(button_to_pin_new[i], INPUT_PULLUP);       //Initialize pins as inputs w/ pullups
